...
export const createPost = async (req, res) => {
  ...
  try {
    ...
    if (error)
      return res.status(400).json({ status: false, message: error.details[0].message, post: {} });

    const titleExist = await PostsModel.findOne({ title: data.title, userID: userID });
    if (titleExist)
      return res.status(401).json({ status: false, message: "Title already exists", post: {} });
...
export const updatePost = async (req, res) => {
  ...
  try {
    ...
    const titleExist = await PostsModel.findOne({
      title: post.title,
      userID: req.user,
      _id: { $ne: id },
    });
...
export const deletePost = async (req, res) => {
  const id = req.params.id;
  try {
    const data = await PostsModel.findOneAndDelete({ _id: id, userID: req.user });
